...
struct MajorManageView: View {
    @ObservedObject var majorDataStore = MajorDataStore()
    ...
    var body: some View {
        ZStack {
            ScrollView {
                VStack {
                    ForEach(0..<majorDataStore.majorData.count, id: \.self, content: { index in
                        VStack(alignment: .leading, spacing: 10.0) {
                            VStack {
                                HStack(spacing: 30.0) {
                                    Text("管理工号")
                                        .font(.system(size: 18))
                                        .bold()
                                    Text("\(majorDataStore.majorData[index].id)")
                                        .font(.system(size: 18))
                                        .bold()
                                }
                            }
                            HStack(spacing: 30.0) {
                                Text("姓名")
                                    .font(.system(size: 18))
                                    .bold()
                                Text("\(majorDataStore.majorData[index].name)")
                                    .font(.system(size: 18))
                                    .bold()
                            }
                            HStack(spacing: 30.0) {
                                Text("最大容纳数量")
                                    .font(.system(size: 18))
                                    .bold()
                                Text("\(majorDataStore.majorData[index].max)")
                                    .font(.system(size: 18))
                                    .bold()
                            }
                        }
            ...
            TitleView(title: "专业信息")
        }
    }
}

struct MajorManageView_Previews: PreviewProvider {
