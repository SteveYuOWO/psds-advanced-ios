...
struct TeacherManageView: View {
    @ObservedObject var teacherDataStore = TeacherDataStore()
    ...
    var body: some View {
        ZStack {
            ScrollView {
                VStack {
                    ForEach(0..<teacherDataStore.teacherData.count, id: \.self, content: { index in
                        VStack(alignment: .leading, spacing: 10.0) {
                            VStack {
                                HStack(spacing: 30.0) {
                                    Text("学号")
                                        .font(.system(size: 18))
                                        .bold()
                                    Text("\(teacherDataStore.teacherData[index].teaNum)")
                                        .font(.system(size: 18))
                                        .bold()
                                }
                            }
                            HStack(spacing: 30.0) {
                                Text("姓名")
                                    .font(.system(size: 18))
                                    .bold()
                                Text("\(teacherDataStore.teacherData[index].name)")
                                    .font(.system(size: 18))
                                    .bold()
                            }
                            HStack(spacing: 30.0) {
                                Text("性别")
                                    .font(.system(size: 18))
                                    .bold()
                                Text("\(teacherDataStore.teacherData[index].sex)")
                                    .font(.system(size: 18))
                                    .bold()
                            }
                            HStack(spacing: 30.0) {
                                Text("专业")
                                    .font(.system(size: 18))
                                    .bold()
                                Text("\(teacherDataStore.teacherData[index].major)")
                                    .font(.system(size: 18))
                                    .bold()
                            }
                        }
            ...
            TitleView(title: "教师信息")
        }
    }
}

struct TeacherManageView_Previews: PreviewProvider {
